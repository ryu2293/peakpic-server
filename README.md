...
![PeakPic AWS 아키텍처](img/architecture.png)
...
terminate-first라 배포 중 실질 1대 서빙 구간이 생기고 있었습니다. `MaxHealthyPercentage 150`으로
launch-before-terminate로 전환한 뒤 기동이 5분 27초 선행하고, 재접속이 새 인스턴스로
반반(25/25) 분산됐습니다.
...
> 배포를 관통해 측정하던 중 측정 노트북의 Wi-Fi가 순단되며 42개 소켓이 코드 없이 끊겼습니다.
...
동시 500세션 **유지**는 여유였습니다. 문제는 동시 **몰림**이었습니다.
...
- **중계 1건당 Redis 순차 5.3왕복** → Lua 스크립트로 정확히 1왕복
  (실측: 중계 48건에 PEXPIRE 146회 → 2회, EVALSHA 48회로 중계 건수와 1:1)
...
![크로스 인스턴스 중계 비율](img/cross-instance.png)

> 운영에서 실제로 중계의 60~72%가 인스턴스를 건너갑니다 — Pub/Sub이 예외 처리가 아니라
> 상시 동작하는 경로임을 보여줍니다.
...
- JOIN 슬롯 경합 → 분산락 대신 **Lua 원자 스크립트**
  — 데이터가 전부 Redis 안에 있어, 락 획득·해제 왕복과 락 수명 관리(TTL, 보유자 사망) 없이
  1왕복으로 끝납니다. 분산락은 임계 구역이 Redis 밖(DB·S3)에 걸칠 때의 도구라고 판단했습니다.
